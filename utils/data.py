...
class IRSTD_Dataset(Data.Dataset):
    def __init__(self, args, mode='train'):
        ...
        # 适配不同的数据集结构
        if osp.exists(osp.join(dataset_dir, 'images')):
            self.imgs_dir = osp.join(dataset_dir, 'images')
        elif osp.exists(osp.join(dataset_dir, 'IRSTD1k_Img')):
            self.imgs_dir = osp.join(dataset_dir, 'IRSTD1k_Img')
        else:
            self.imgs_dir = osp.join(dataset_dir, 'images')  # 默认值
        
        if osp.exists(osp.join(dataset_dir, 'masks')):
            self.label_dir = osp.join(dataset_dir, 'masks')
        elif osp.exists(osp.join(dataset_dir, 'IRSTD1k_Label')):
            self.label_dir = osp.join(dataset_dir, 'IRSTD1k_Label')
        else:
            self.label_dir = osp.join(dataset_dir, 'masks')  # 默认值
